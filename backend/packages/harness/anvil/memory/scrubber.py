...
class MemorySecretScrubber:
    """Deterministic secret scrubber used before HCMS persistence or export."""

    _TOKEN_RULES: tuple[tuple[str, re.Pattern[str], str], ...] = (
        ("github_token", re.compile(r"\b(?:ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9_]{20,}\b"), "[REDACTED:github_token]"),
        ("openai_project_token", re.compile(r"\bsk-proj-[A-Za-z0-9_-]{16,}\b"), "[REDACTED:openai_project_token]"),
        ("openrouter_token", re.compile(r"\bsk-or-v1-[A-Za-z0-9_-]{16,}\b"), "[REDACTED:openrouter_token]"),
        ("api_key", re.compile(r"\bsk-[A-Za-z0-9_-]{12,}\b"), "[REDACTED:api_key]"),
        ("bearer_token", re.compile(r"(?i)\bBearer\s+(?!\[REDACTED:)[A-Za-z0-9._~+/=-]{24,}\b"), "Bearer [REDACTED:bearer_token]"),
    )
    _ASSIGNMENT_RULES: tuple[tuple[str, re.Pattern[str], Callable[[re.Match[str]], str]], ...] = (
        (
            "secret_assignment",
            re.compile(r"(?i)\b((?:[A-Z0-9_]*(?:API[_-]?KEY|TOKEN|SECRET|CREDENTIAL)[A-Z0-9_]*))\s*[:=]\s*(?!\[REDACTED:)([^\s,;\"']{8,})"),
